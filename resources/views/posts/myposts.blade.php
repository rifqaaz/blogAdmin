...
<!-- Categories Section -->
<div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mb-4 justify-content-center">
    <div class="p-4 text-gray-900">
        @if($categories->count())
            <div class="d-flex flex-wrap gap-2 mb-4">
                @foreach($categories as $category)
                    <a href="{{ route('posts.index', ['category' => $category->id]) }}" 
                    class="badge rounded-pill p-2 px-3 text-decoration-none 
...
<div class="mb-4 d-flex justify-content-between align-items-center">
    <!-- Show Active Toggle -->
    <div class="mb-4">
        <a href="{{ request()->fullUrlWithQuery(['show_active' => request('show_active') ? null : '1']) }}" 
        class="btn btn-sm {{ request('show_active') ? 'btn-danger' : 'btn-outline-secondary' }}">
            {{ request('show_active') ? 'Hide Active Posts' : 'Show Active Posts' }}
        </a>
    </div>
    
    <!-- New Post Button -->
     @can('create posts')
    <a href="#" class="btn btn-primary btn-sm text-decoration-none"  data-toggle="modal" data-target="#create">New Post</a>
    @endcan
</div>
@endauth
...
<div class="container-fluid px-0">
    <div class="row row-cols-1 row-cols-md-2 row-cols-lg-3">
        ...
        <div class="col-lg-4 mb-4"> <!-- Added mb-4 for spacing -->
            <div class="card h-100"> <!-- Ensures equal height -->
                <div class="card card-custom">
                    ...
                    <div class="card-body d-flex flex-column">
                        @can('edit assigned')<p>Author: {{ $post->user->name }}</p>@endcan
